...
class SellerOrderRow(BoxLayout):
    def __init__(self, order, on_status_change, on_delete, **kwargs):
        ...
        top_row.add_widget(Label(
            ...
            color=(0.1, 0.1, 0.1, 1), font_size="15sp", bold=True
        ))
        top_row.add_widget(Label(text=f"${order['total_amount']}", color=(0.3, 0.5, 0.9, 1), font_size="15sp"))
        self.add_widget(top_row)

        items_text = ", ".join(f"{item['name']} x{item['quantity']}" for item in order["items"])
        self.add_widget(Label(text=items_text, color=(0.4, 0.4, 0.4, 1), font_size="12sp", size_hint=(1, 0.25)))

        status_row = BoxLayout(size_hint=(1, 0.45), spacing=6)
        status_row.add_widget(Label(text=f"Status: {order['status']}", color=(0.2, 0.2, 0.2, 1), size_hint=(0.4, 1)))
        ...
        delete_button = Button(text="Delete", size_hint=(0.3, 1), background_color=(0.8, 0.3, 0.3, 1), background_normal="", color=(1, 1, 1, 1))
...
class SellerOrdersScreen(Screen):
    def __init__(self, **kwargs):
        ...
        header.add_widget(Label(text="My Store Orders", font_size="20sp", bold=True, color=(0.2, 0.3, 0.6, 1)))
        ...
        self.orders_layout = BoxLayout(orientation="vertical", spacing=10, size_hint_y=None, padding=[0, 5, 0, 5])
